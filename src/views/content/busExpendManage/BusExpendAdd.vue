    ...
    <el-row style="margin-top: 20px">
      <el-col :span="12" :offset="4">
        <el-form :model="form" :rules="rules">
          ...
          <el-form-item label="费用类型" label-width="110px" prop="expendTypeName">
             <el-select
          ...
          <el-form-item>
            <el-row>
              <el-col :span="16" :offset="8">
                <el-button type="primary" @click="$manage.add(vc, 'busExpend', form)">添加数据</el-button>
...
export default {
  ...
  methods: {
    querySearch(queryString, cb) {
        ...
        var results = queryString ? restaurants.filter(this.$manage.createFilter(queryString)) : restaurants;
        cb(results);
      },
    cancle() {
      this.$router.go(-1);
  ...
  mounted() {
    ...
    //请求审批人用户名列表
    this.$api.busExpend.getApproverUserNames()
    .then(response => {
      response.data.data.forEach(item => {
